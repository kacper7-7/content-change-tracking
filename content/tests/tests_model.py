...
class FollowModelTest(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            email="user5556@example.com", password="password"
        )
    ...
    def test_user_cannot_follow_same_content_twice(self):
        ...
        with self.assertRaises(IntegrityError):
            Follow.objects.create(user=self.user, content=self.content)
    ...
    def test_last_viewed_at_is_timezone_now(self):
        ...
        content_2 = Content.objects.create(
            author=self.user, title="Title-Test", body="Body-Test"
        )

        follow_2 = Follow.objects.create(user=self.user, content=content_2)
